  
const generateHtml = require('../src/htmlgen');
const Manager = require('./Manager');
const inquirer = require('inquirer');
const Engineer = require('./Engineer');
const Intern = require('./Intern');

class Team {
    constructor() {
    this.team = [];
    }
    
    // create employee, starting with manager by default
    addEmployee(position, roleSpecific) {
        inquirer.prompt([{
                type: 'input',
                name: 'name',
                message: `Enter ${position} name:`,
                validate: nameInput => {
                    if (nameInput) {
                        return true;
                    } else {
                        console.log('Please enter your name');
                        return false;
                    }
                }
            }, 
            {
                type: 'input',
                name: 'id',
                message: `Enter ${position} ID:`,
                validate: idInput => {
                    if (idInput) {
                        return true;
                    } else {
                        return false;
                    }
                }
            }, 
            {
                type: 'input',
                name: 'email',
                message: `Enter ${position} email:`,
                validate: emailInput => {
                    if (emailInput) {
                        return true;
                    } else {
                        console.log('Please enter a valid email address');
                        return false;
                    }
                }
            }, 
            {
                type: 'input',
                name: 'position',
                message: `Enter ${position} ${roleSpecific}:`,
                validate: roleSpecificInput => {
                    if (roleSpecificInput) {
                        return true;
                    } else {
                        console.log(`Please enter ${roleSpecific}`);
                        return false;
                    }
                }
            }])
            .then(answers => {
                const {name,id,email,position} = answers;
                // check position of employee
                if (position === 'manager') {
                    let newEmployee = new Manager(name, id, email, position);
                    return newEmployee;
                } else if (position === 'intern') {
                let newEmployee = new Intern(name, id, email, position);
                return newEmployee;
                } else {
                    let newEmployee = new Engineer(name, id, email, position);
                return newEmployee;
                }
            }).then(newEmployee => {
                this.team.push(newEmployee);
                // check if adding more employees
                return this.addToTeam();
            })
            .catch(error => {
                console.log(error);
            });
    }
    // check if user wants to add more employees
    addToTeam() {
        inquirer.prompt({
            type: 'list',
            name: 'positionSelect',
            message: 'Would you like to add another employee?',
            choices: ["Engineer", "Intern", "I don't want to add anymore team members"]
        }).then(answer => {
            const {positionSelect} = answer;
            if (positionSelect === 'Engineer') {
                return this.addEmployee('engineer', 'GitHub Username');
            } else if (positionSelect === 'Intern') {
                return this.addEmployee('intern', 'school');
            } else {
                return generateHtml(this.team);
            }
        });
    };
};

module.exports = Team;